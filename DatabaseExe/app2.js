...
데이터베이스를 테이블이나 엑셀처럼 쉽게 다룰 수 있게하는 모듈
Schema(스키마) :
String,Number,Boolean,Array,Buffer,Date,ObjectId,Mixed
어떤 문서(컬럼)에는 name이 있고 다른 문서에는 name이 없을 수도 있기 때문에
일정한 조건으로 적용하기가 힘들다(어렵다).
그래서 관계형 DB(Oracle)처럼 조회 조건을 공통적으로 적용할수있게
정해진 규칙으로 문서를 저장할 수 있게 하는것이 Schema이다.
 */ 
