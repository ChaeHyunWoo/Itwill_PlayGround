...
Schema.createSchema = function(mongoose){
	...
	UserSchema = mongoose.Schema({
		email:{type:String,required:true,unique:true}, // 아이디에 들어가는 값은 문자고 required 반드시 필요하고 , 유의값 - > 프라이머리키가 됨
	...
	//로그인에서 사용
	// findByEmail는 메소드의 이름 뒤에 콜백함수 
	
	// static를 써서 메소드 
	UserSchema.static("findByEmail",function(email,callback){ // 이메일를 가지고 select 했을때 들어갔을때 무조건 작업이 끝났을때 결과가 콜백으로 간다.
		return this.find({email:email},callback); // email를 찾아라 찾던안찾던 callback함수 실행해라
	})
